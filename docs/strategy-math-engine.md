...
策略数学层仍然只输出交易意图，不直接管理订单、保证金、仓位表或行情拉取。多币种、做多、做空、杠杆和保证金计算属于 SaaS 回测执行层；策略包不得为了回测增加 `isBacktest`、数据库访问、网络访问或交易所 SDK。

当回测方向模式为 `long` 时，买入意图表示开多或加多，卖出意图表示平多或减多。当方向模式为 `short` 时，回测执行层只能接受明确标记为空头的开平仓意图；没有空头语义的普通现货卖出意图不得被偷偷解释为真实做空策略。当方向模式为 `long_short` 时，执行层必须分别维护多头与空头持仓、保证金和盈亏，不能把多空混成单一现货仓位。

杠杆只影响回测虚拟执行层的名义价值、保证金占用、手续费基数、未实现盈亏、已实现盈亏和预估强平价；它不得改变 `Step()` 的纯计算输入输出契约。系统级杠杆上限为 `100x`，任何策略参数、前端输入或 HTTP 请求都不得突破该上限。

用户界面可以提供多币种、周期、时间范围、方向模式和杠杆选择，但界面文案必须使用用户可理解的业务描述。裸露的 `Step()`、内部仓位状态机名称、公式变量或无上下文英文原因码只能出现在审计与诊断场景，不能作为普通用户配置项。

`ColdSealedBTC` 永不释放。`STRESS_CRASH` 或 `BEAR_TREND` 下默认禁止释放，避免在恐慌阶段把底仓交给微观卖出；如未来需要紧急偿付或人工干预，必须先扩展本文档，不得在代码中临时开口子。

## 第 8 章：可进化参数契约（Chromosome）
